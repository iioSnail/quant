...
def DaYangXian_LowTurnoverRate(
        stock_code,
        turnover_rate=(3, 4),
    ...
    data = pd.concat([daily_data, daily_extra_data], axis=1)
    ...
    curr_data = curr_data[(min_turnover <= curr_data['turnover_rate']) & (curr_data['turnover_rate'] < max_turnover)]
...
class DaYangXianLowTurnover_AnalysisOne(AnalysisOne):
    """
    当出现大阳线时，通常伴随高换手率。
    ...
    def filter_data(self, data: DataFrame, turnover_rate=(3, 4), *args, **kwargs):
        # 获取该股票历史上的大阳线数据
        data['day_chg'] = (data['close'] - data['open']) / data['open'] * 100
